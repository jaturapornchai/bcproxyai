...
| 🤖 Agent-ready | Hermes agent + OpenClaw + Aider + Cline ใช้ได้ 100% — Mistral message-order auto-patch + tool_calls passthrough (13/16 Hermes tools pass clean) |
...
12. **Mistral message-order auto-patch** — inject `assistant()` turn ก่อน `tool` ที่มา after `system`/`user` เพื่อให้ผ่าน Mistral chat-template validator (Hermes agent + OpenClaw ใช้ได้ 100%)
13. **Client-400 short-circuit** — upstream คืน 400 invalid-shape → หยุด retry ทันที surface error กลับ client (ไม่ burn 10+ retries บน bug ฝั่งลูก)
14. **Parallel skip-checks** — Promise.all รวม cooldown/TPM/fit/capacity/circuit check (เดิม serial 7 ตัว)
15. **Connection pre-warm** — ping top providers ตอน boot + ทุก 4 นาที (keep-alive socket ไม่ตาย)
...
hermes config set model.default mistral/mistral-small-latest  # tool_calls stable
...
Gateway auto-patches `[system, tool, ...]` message order for Mistral → Hermes tool flow works out-of-the-box. ทดสอบแล้ว 13/16 built-in tools pass clean (terminal, file RW, code_execution, todo, vision, tts, clarify, session_search, skills, delegation, moa, ฯลฯ).
