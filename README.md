...
**OpenAI-compatible LLM gateway ที่ใช้เฉพาะโมเดลฟรีแบบ hardcoded**
...
**DB-driven keys** — API keys อยู่ใน database (`api_keys` table). `.env.local` ใช้แค่ runtime/auth config — **ไม่อ่าน API key จาก env**. ตั้งค่า key ผ่าน Setup modal ในหน้า dashboard.

**Cost policy** — ระบบบล็อกทุกอย่างที่ไม่อยู่ใน `src/lib/free-model-catalog.ts`. ใช้เฉพาะ OpenRouter model id ที่ลงท้าย `:free` และอยู่ใน hardcoded catalog เท่านั้น. Local/Ollama, provider-wide allowlist, router alias เช่น `openrouter/free`, และ paid override ถูกปิดทั้งหมด. ถ้า free quota หมดหรือชน limit provider จะตอบ error/limit กลับมา แล้ว SMLGateway จะ cooldown provider/model นั้นและกลับมาใช้ใหม่เองเมื่อพร้อม. แนะนำใช้ OpenRouter account ที่ไม่ผูกบัตร/ไม่มี billing เพื่อกัน provider ตัดเงินเอง.

**Hardcoded catalog** — worker ไม่ค้นหา provider/model จาก internet แล้ว. รอบ worker จะ sync เฉพาะรายการ model ฟรีใน `src/lib/free-model-catalog.ts` เข้า database เพื่อ health/exam/routing.

## สิ่งที่ Dev ได้ทันที

| | |
|---|---|
| 🆓 No-spend default | ใช้เฉพาะ hardcoded OpenRouter `:free` models; local/Ollama, provider-wide allowlists, router alias, และ paid override ถูกบล็อกแม้มี key |
| 🇹🇭 Thai routing | virtual `sml/thai` เลือกจาก hardcoded free catalog ตามคะแนน/ผลใช้งานจริง |
...
| 📌 Hardcoded catalog | model ที่ใช้ได้อยู่ใน source code เท่านั้น ไม่สแกน provider/model อัตโนมัติ |
...
| 🔍 Model search | `/v1/models/search` filter เฉพาะ hardcoded free catalog |
...
|---|---|---|
| main | 15 นาที | sync hardcoded free catalog + health + exam + teacher |
...
ใช้ `/setup` ใส่ OpenRouter API key เท่านั้น. Runtime resolver ใช้ endpoint hardcoded ใน [src/lib/providers.ts](src/lib/providers.ts) และอนุญาตเฉพาะ model ใน [src/lib/free-model-catalog.ts](src/lib/free-model-catalog.ts).

Local/Ollama, provider ที่ไม่อยู่ใน catalog, และ model alias แบบ auto-router ถูกบล็อกใน cost policy.
...
เรียก model ตรงได้เฉพาะตัวที่อยู่ใน hardcoded free catalog เช่น `openrouter/openai/gpt-oss-20b:free` หรือ `openrouter/qwen/qwen3-coder:free`
...
| `GET  /v1/models/:id` | ดึงข้อมูล model ตัวเดียว รองรับ ID ที่มี `/` เช่น `sml/tools`, `openrouter/openai/gpt-oss-20b:free` |
...
| `GET  /api/provider-catalog` | รายการ provider catalog จาก DB สำหรับหน้า dashboard |
| `POST /api/provider-catalog` | ปิดใช้งานแล้ว เพราะระบบใช้ hardcoded free catalog |
...
```
prefer:          ["openrouter"]        ดัน provider นี้ขึ้นบน (ตอนนี้มี provider เดียว)
exclude:         ["openrouter"]        ตัดทิ้งเพื่อทดสอบ no-candidate path
...
```bash
curl -X POST http://localhost:3334/v1/chat/completions \
  -H "X-SMLGateway-Prefer: openrouter" \
...
curl -X POST http://localhost:3334/v1/compare \
  -d '{"messages":[...],"models":["openrouter/openai/gpt-oss-20b:free","openrouter/qwen/qwen3-coder:free"],"max_tokens":200}'
```
...
| `SSRF_ALLOW_PRIVATE` | _(unset)_ | `1` | dev only — disable SSRF guard ใน `/api/admin/providers/*/test` |

Cost policy ไม่มี env override แล้ว. รายการที่ route ได้อยู่ใน `src/lib/free-model-catalog.ts` เท่านั้น.
