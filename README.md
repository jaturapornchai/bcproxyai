...
**Cost policy** — ค่าเริ่มต้นบล็อก provider ที่อาจคิดเงินทั้งหมด แม้มี API key อยู่ใน DB. อนุญาตเฉพาะ `ollama` และ `pollinations` ผ่าน `SML_FREE_PROVIDER_ALLOWLIST`. ถ้าจะใช้ cloud free tier ต้องเพิ่มชื่อ provider เองหลังยืนยันว่า deployment นั้นไม่มีทางถูกคิดเงิน; paid deployment ต้องตั้ง `SML_ALLOW_PAID_PROVIDERS=1`.
...
| | |
|---|---|
| 🆓 No-spend default | ใช้งานจริงเฉพาะ provider ใน `SML_FREE_PROVIDER_ALLOWLIST` (`ollama,pollinations` ค่าเริ่มต้น); cloud provider อื่นถูกบล็อกแม้มี key |
...
| `SML_FREE_PROVIDER_ALLOWLIST` | `ollama,pollinations` | CSV provider names | รายชื่อ provider ที่อนุญาตให้ยิง upstream ใน no-spend mode |
