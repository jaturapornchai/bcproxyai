...
async function recordCircuitFailure(provider: string): Promise<void> {
  try {
    ...
    if (total >= 5) {
      ...
      if (successRate < 0.3) {
        // Open circuit for 30s (smart recovery)
        const cbKey = `cb:open:${provider}`;
...
async function forwardToProvider(
  ...
  // Per-attempt timeout — dynamic based on body size
...
async function hedgeRace(
  ...
  body: Record<string, unknown>,
  isStream: boolean
): Promise<{ response: Response; winner: ModelRow; winnerIdx: number; loserIndices: number[] }> {
  ...
  const attempts = candidates.map((candidate, idx) =>
    forwardToProvider(candidate.provider, candidate.model_id, body, isStream, controllers[idx].signal)
      .then(res => {
        if (res.ok) {
          const loserIndices = candidates.map((_, j) => j).filter(j => j !== idx);
          return { response: res, winner: candidate, winnerIdx: idx, loserIndices };
        }
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Context-aware provider filter — for large prompts require 1.5x headroom
    if (estInputTokens > 20_000) {
      const beforeCount = finalCandidates.length;
      const ctxFiltered = finalCandidates.filter(c => c.context_length >= estInputTokens * 1.5);
      if (ctxFiltered.length > 0) {
        finalCandidates = ctxFiltered;
        console.log(`[CTX-FILTER:${_reqId}] kept=${finalCandidates.length}/${beforeCount} for ${estInputTokens}tok`);
      } else {
    ...
    if (
      ...
      try {
        ...
        const { response: hedgeResp, winner, loserIndices } = hedgeResult;
        const losers = loserIndices.map(j => topThree[j]);
        const latency = Date.now() - startTime;
        ...
        const latency = Date.now() - startTime;
        console.log(`[HEDGE-LOSS:${_reqId}] all top-${topThree.length} failed | ${latency}ms — continuing sequential`);
        await Promise.all(topThree.flatMap(c => [
          recordProviderFailureMem(c.provider),
          recordCircuitFailure(c.provider),
        ]));
      }
      // After hedge (win bad-response or loss), skip hedge candidates in sequential loop
      hedgeStartIdx = topThree.length;
    }
