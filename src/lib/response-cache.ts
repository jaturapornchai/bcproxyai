...
// Response cache is ENABLED by default (aggressive mode).
//
// Cached for 1 hour keyed by sha256 of {model, messages, temperature, tools,
// tool_choice}. Because the full messages array and tool definitions are part
// of the hash, cross-response bleeding between different conversations is
// mitigated — two requests only collide if every field is byte-identical.
//
// Streaming responses are never cached (streaming cache doesn't work).
//
// Disable entirely by setting RESPONSE_CACHE_ENABLED=0 in the environment.
